...
const generateAccessToken = async () => {
  try {
    if (!EnvVars.Paypal.clientID || !EnvVars.Paypal.clientSecret) throw new Error("MISSING_API_CREDENTIALS");
    const auth = Buffer.from(EnvVars.Paypal.clientID + ":" + EnvVars.Paypal.clientSecret,).toString("base64");
